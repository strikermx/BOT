...
sub modifyPacketIn {
	...
	#	my $accountInfo = substr($msg, 0, 147);
	#	my $serverInfo = substr($msg, 147, length($msg));
	#	my $newServers = '';
	#	my $serverCount = 0;
	...
	if ($switch eq "0069" || $switch eq "0276") {# 
		...
		for (my $i = 0; $i < $msg_size; $i+=32) {
			if ($config{'server'} == $serverCount++) {
				...
				#nOoiTz $newServers .= encodeIP($self->{proxy}->sockhost) . pack("v*", $self->{proxy}->sockport) . 
				$newServers .= encodeIP($self->{nextIp}) . pack("v*", $self->{nextPort}) .  
		...
		$msg = $logonInfo . encodeIP($self->{nextIp}) . pack("v*", $self->{nextPort});
